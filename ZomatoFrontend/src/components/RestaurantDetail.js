...
useEffect(() => { 
fetch(`https://zomatoserver.herokuapp.com/restaurant/details/${rName}`,{method:'GET'}) 
.then(response=>response.json()) 
.then(data=>setRestaurant(data.data)) 
...
const fetchMenu=()=>{ 
fetch(`https://zomatoserver.herokuapp.com/menu/${rName}`,{method:'GET'}) 
.then(response=>response.json()) 
.then(data=>setMenu(data.data)) 
} 
...
const openRazorpay=async()=>{
  ...
  orderData = await fetch('https://zomatoserver.herokuapp.com/pay',{
    method:'POST',
...
// open razorpay window
  const options={ 
  key:"rzp_test_4rHa7PGErVmiHv"  ,  /*change this key to your key */
  ...
  handler:function(response){
    ...
    fetch('https://zomatoserver.herokuapp.com/pay/save',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
